...
export const FillTextEditor: React.FC<QuestionEditorProps> = ({
  question,
  onUpdate,
}) => {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      >
        ...
        <textarea
          ...
          placeholder="Use *** to indicate blanks (e.g., The capital of France is ***)"
          rows={4}
      ...
      >
        <label className="block text-sm font-geist-mono text-white/70 mb-2">Answer</label>
        <textarea
          value={question.correctAnswer as string || ''}
          onChange={(e) => onUpdate({ correctAnswer: e.target.value })}
          placeholder="Enter the correct answer for the blanks (e.g., Paris)"
          rows={4}
          className="w-full px-3 py-2 bg-black/20 border border-white/10 rounded-lg text-white placeholder-white/40 focus:border-white/20 focus:outline-none focus:ring-2 focus:ring-white/20 transition-all duration-200 font-geist-mono resize-none"
        />
      </motion.div>
